...
The working tree these patches come from is public at [cornch-k/llama.cpp](https://github.com/cornch-k/llama.cpp/tree/local-latest) (branch `local-latest`, currently `6eecc1736`), a fork of upstream. It is a research fork and is not proposed upstream: llama.cpp does not accept predominantly AI-assisted contributions, and the E2 patch was written that way. The ggml scheduler defect described in Appendix A(8) is reported there as an observation, not as a submitted fix.

| File | What | Base |
|---|---|---|
| `llama.cpp-e2-fold.patch` | The **expert-only** fold of §4.1 and everything §4.5 and Table 3's Metal row depend on: `LLAMA_FOLD_EXPERTS=R` / `LLAMA_FOLD_MAP` fold the FFN block only (router, routed experts, shared expert, post-attention norm) while the mixer keeps its logical layer, plus `LLAMA_FOLD_SHEXP_LOGICAL`, `LLAMA_FOLD_GAMMA[_FILE]`, `LLAMA_FOLD_NORM_LOGICAL`, and the two loader knobs that make Metal execution possible (`LLAMA_FOLD_SKIP_LOAD`, `LLAMA_MMAP_GPU_COPY`). Includes the new `src/models/fold.h`. laguna + qwen35moe architectures. | [cornch-k/llama.cpp@`6eecc1736`](https://github.com/cornch-k/llama.cpp/tree/local-latest) |

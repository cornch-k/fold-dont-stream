...
run() { # $1=tag $2=model
  ...
  local _unused=$(grep -ao 'Generation: [0-9.]* t/s' $EXP/e58-$TAG-spd.log | tail -1 | sed 's/Generation: //')
  local MEM=$(python3 $EXP/memcalc58.py $EXP/e58-$TAG-ppl.log "$M")
  echo "RESULT $TAG  ppl=$PPLV  속도=$SPD  $MEM  wired=$PW2"
}

run P1 models/qwen35-v2a-hot.gguf
run P2 models/qwen35-v2a-hot.gguf
run P3 models/qwen35-v2a-hot.gguf
run P4 models/qwen35-v2a-hot.gguf
run P5 models/qwen35-v2a-hot.gguf

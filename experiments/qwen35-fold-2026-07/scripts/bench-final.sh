...
run() { # $1=모델 $2=fold환경("-"이면 없음) $3=벤치인자 $4=k-override("-"이면 없음) $5=태그 $6=최대반복
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] EXP-35 $5 시작" >> $EXP/timeline.log
  env LLAMA_ARG_REPACK=false LLAMA_MMAP_NO_PREFETCH=1 GGML_OP_OFFLOAD_MIN_BATCH=999999 $ENV1 \
      "$BIN" -m "$1" -ngl 99 --no-repack -c 2048 -b 2048 -ub 512 --seed 1337 $KV \
      $3 > "$LOG" 2>&1 &
...
for MODEL_TAG in "folded" "rival"; do
  ...
  run "$MDL" "$FE" "--multiple-choice --multiple-choice-tasks 2000 -f $EXP/mmlu-test.bin" "$KO" "$MODEL_TAG-mmlu" 700

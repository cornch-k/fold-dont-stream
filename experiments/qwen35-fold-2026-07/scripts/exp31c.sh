#!/bin/bash
# EXP-31c v2a(imatrix+혼합정밀) ppl. 비교: v1 12층 = 20.3964 ± 0.89558
set -u
KILL_GB=22
...
IFS=,; MAP="${m[*]}"; unset IFS
echo "[$(date '+%Y-%m-%d %H:%M:%S')] EXP-31c v2a ppl" >> $EXP/timeline.log
env LLAMA_ARG_REPACK=false LLAMA_MMAP_NO_PREFETCH=1 GGML_OP_OFFLOAD_MIN_BATCH=999999 \
    ...
    --override-kv qwen35moe.expert_used_count=int:6 > $EXP/e31c.log 2>&1 &
PID=$!; PEAK=0; i=0
...
wait $PID 2>/dev/null
echo "v1 12층 : 20.3964 +/- 0.89558"
echo "v2a 12층: $(grep -a 'Final estimate' $EXP/e31c.log | tail -1 | sed 's/.*PPL = //')   peak ${PEAK} GB"
